...
struct Player {
  ...
  Player(uint64_t id, float x, float y, float z, float rotX, float rotY,
         float rotZ)
      : id(id), x(x), y(y), z(z), rotationX(rotX), rotationY(rotY),
        rotationZ(rotZ) {
    // Create a temporary LveGameObject and then move it to the heap for
    // unique_ptr to manage.
    auto tempGameObject = lve::LveGameObject::createGameObject();
    gameObject = std::unique_ptr<lve::LveGameObject>(
        new lve::LveGameObject(std::move(tempGameObject)));
