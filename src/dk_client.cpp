...
// The following only works on Windows for now, very easy to port to linux
...
//
//       memcpy(sendBuff + offsetxd, &players[0].rotationX,
//              sizeof(players[0].rotationX));
//       offsetxd += sizeof(players[0].rotationX);
//
//       memcpy(sendBuff + offsetxd, &players[0].rotationY,
//              sizeof(players[0].rotationY));
//       offsetxd += sizeof(players[0].rotationY);
//
//       memcpy(sendBuff + offsetxd, &players[0].rotationZ,
//              sizeof(players[0].rotationZ));
...
//         std::lock_guard<std::mutex> lock(sync);
//
//         auto it = std::find_if(dk::players.begin(), dk::players.end(),
...
//           dk::players.push_back(
//               Player(playerID, x, y, z, rotationX, rotationY, rotationZ));
//         }
//       }
//       dk::players.erase(
//           std::remove_if(dk::players.begin(), dk::players.end(),
//                          [&receivedPlayerIDs](const Player &player) {
//                            return receivedPlayerIDs.find(player.id) ==
//                                   receivedPlayerIDs.end();
//                          }),
//           dk::players.end());
...
//         std::chrono::milliseconds(500)); // for now testing
//   }
//   std::cout << "[CLIENT THREAD] Loop has been broken. Closing connection and
//   "
//                "thread..."
//             << std::endl;
//   std::lock_guard<std::mutex> lock(sync);
...
void drawPlayers(VkCommandBuffer commandBuffer, const lve::LveCamera &camera,
                 VkPipelineLayout pipeline, lve::LveDevice &device,
                 lve::LveGameObject &ground) {
  ...
  //     return;
  //   }
  //   // Skip first index as this is local player, and is drawn in dk_car,
  //   perhaps I
  //   // ought ot merge the two but I rlly cba wanna finish this asap. Can do
  //   later for (int i = 1; i < players.size(); i++) {
  //
  ...
  //     player.gameObject->transform.translation =
  //         glm::vec3(player.x, -1.f, player.z);
  //     player.gameObject->transform.rotation =
  //         glm::vec3(player.rotationX, player.rotationY, player.rotationZ);
  ...
  //
  //     vkCmdPushConstants(commandBuffer, pipeline,
  //                        VK_SHADER_STAGE_VERTEX_BIT |
  //                            VK_SHADER_STAGE_FRAGMENT_BIT,
  //                        0, sizeof(SimplePushConstantData), &carPush);
