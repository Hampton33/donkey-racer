  ...
  while (!lveWindow.shouldClose()) {
    ...
    if (frame % 10 == 0 && client.isThreadRunning) {
      ...
      client.updatePos(car.carGameObject.transform.translation,
                       car.carGameObject.transform.rotation);
    }
    glm::vec3 cameraPosition =
        car.carGameObject.transform.translation + cameraOffset;

    glfwPollEvents();
    handle_input(lveWindow.getWindow(), car);
    camera.setViewTarget(cameraPosition,
                         car.carGameObject.transform.translation,
...
void DkApp::loadGameObjects() {
  ...
  std::shared_ptr<LveModel> skyModel =
      LveModel::createModelFromFile(lveDevice, "models/skybox.obj");
  ...
  std::shared_ptr<LveModel> carModel =
      LveModel::createModelFromFile(lveDevice, "models/ghetto_car.obj");
